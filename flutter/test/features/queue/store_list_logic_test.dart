...
void main() {
  group('sortStoreEntries', () {
    test('pinned stores come first, then today orders, then history', () {
      final pinned = StoreListEntry(
        store: _store('store-1', 'مخبز الرمال'),
        pinned: true,
      );
      final today = StoreListEntry(
        store: _store('store-2', 'مخبز الشاطئ'),
        todayStatus: PurchaseStatus.waiting,
        lastPurchaseDate: '2026-09-05',
      );
      final history = StoreListEntry(
        store: _store('store-3', 'مخبز النصيرات'),
        lastPurchaseDate: '2026-09-01',
      );
      final fresh = StoreListEntry(store: _store('store-4', 'مخبز الأمل'));

      final sorted = sortStoreEntries([fresh, history, today, pinned]);

      expect(sorted.map((e) => e.store.id).toList(), ['store-1', 'store-2', 'store-3', 'store-4']);
    });
    ...
    test('within a group, the most recent purchase wins', () {
      ...
      expect(sorted.map((e) => e.store.id).toList(), ['store-2', 'store-1']);
    });

    test('otherwise alphabetical by store name', () {
      final a = StoreListEntry(store: _store('store-1', 'أ'));
      final b = StoreListEntry(store: _store('store-2', 'ب'));
    ...
    test('does not mutate the input list', () {
      final input = [
        StoreListEntry(store: _store('store-1', 'ب')),
        StoreListEntry(store: _store('store-2', 'أ')),
      ];
  ...
  group('filterStoreEntries', () {
    final entries = [
      StoreListEntry(store: _store('store-1', 'مخبز الرمال', area: 'الرمال')),
      StoreListEntry(store: _store('store-2', 'مخبز الشاطئ', area: 'الشاطئ')),
      StoreListEntry(store: _store('store-3', 'مخبز النصيرات', area: 'النصيرات')),
    ];

    test('filters by store name', () {
      final visible = filterStoreEntries(entries, query: 'الشاطئ');

      expect(visible.map((e) => e.store.id).toList(), ['store-2']);
    });

    test('filters by area name too', () {
      final visible = filterStoreEntries(entries, query: 'النصيرات');

      expect(visible.map((e) => e.store.id).toList(), ['store-3']);
    });

    test('filters by selected area chip', () {
      final visible = filterStoreEntries(entries, area: 'الرمال');

      expect(visible.map((e) => e.store.id).toList(), ['store-1']);
    });
  ...
  group('areasOf', () {
    test('returns distinct non-empty areas in first-seen order', () {
      final entries = [
        StoreListEntry(store: _store('store-1', 'أ', area: 'الرمال')),
        StoreListEntry(store: _store('store-2', 'ب', area: '')),
        StoreListEntry(store: _store('store-3', 'ج', area: 'الشاطئ')),
        StoreListEntry(store: _store('store-4', 'د', area: 'الرمال')),
      ];
